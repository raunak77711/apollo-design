...
// Element-level keys (as opposed to nested `properties`). `hidden` is a
// layer visibility flag toggled from the Layers panel.
...
export function applyOperation(doc, op) {
  ...
  switch (op.type) {
    ...
    case 'DUPLICATE_ELEMENT': {
      ...
      if (!src) return doc;
      const copy = {
        ...JSON.parse(JSON.stringify(src)), id: genId(src.type),
